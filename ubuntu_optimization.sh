...
echo "* hard nofile 65535" | sudo tee -a /etc/security/limits.d/custom-limits.conf
echo "* soft nofile 65535" | sudo tee -a /etc/security/limits.d/custom-limits.conf

# 调整内核参数 通过 here 文档（heredoc）向 /etc/sysctl.d/99-custom.conf 文件中添加内核参数
sudo bash -c 'cat << EOF >> /etc/sysctl.d/99-custom.conf
# 调整网络参数 增加 TCP 最大连接数
