...
if grep -q 'ExecStart=/opt/nezha/agent/nezha-agent -s nzip.moneytaoist.sbs:5555 -p [^ ]*  --disable-auto-update' "$SERVICE_FILE"; then
    echo "匹配到第一种 ExecStart 格式，执行相关命令..."
    source ~/.bashrc && \
...
elif grep -q 'ExecStart=/opt/nezha/agent/nezha-agent "-s" "nzip.moneytaoist.sbs:5555" "-p" "[^"]*"  --disable-auto-update' "$SERVICE_FILE"; then
    echo "匹配到第二种 ExecStart 格式，执行相关命令..."
    source ~/.bashrc && \
    sudo sed -i 's|nzip.moneytaoist.sbs:5555|www.money-taoist.icu:443|g' "$SERVICE_FILE" && \
    sudo sed -i 's|--disable-auto-update|"--tls" --disable-auto-update|g' "$SERVICE_FILE" && \
    sudo /usr/bin/systemctl daemon-reload && \
